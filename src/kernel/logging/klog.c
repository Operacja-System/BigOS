...
static const char* overflow_warning = "[WARNING]Some of the logs were lost due to overflow";
...
static void put_overflow_char(ring_buffer_t* buf, const char c) {
	i32 temp_write = buf->write;
	buf->ring[temp_write++] = c;
	temp_write %= RING_BUF_SIZE;
}

static void put_overflow_warning(ring_buffer_t* buf) {
	size_t it = 0;
	while (overflow_warning[it]) put_overflow_char(buf, overflow_warning[it++]);
}

static void put_msg(ring_buffer_t* buf, const char* msg, const int msg_len) {
	put_string(buf, msg);
	if (buf->overflown)
...
static void klogv(klog_severity_level_t loglvl, const char* fmt, va_list va) {
	...
	va_list copy;
	va_copy(copy, va);
	int n = stbsp_vsnprintf(temp_buffer + written, sizeof(temp_buffer) - written, fmt, copy);
	va_end(copy);
